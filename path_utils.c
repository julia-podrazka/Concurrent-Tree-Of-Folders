// File provided by the author of a project.
...
bool is_path_valid(const char *path) {
    ...
    if (len == 0 || len > MAX_PATH_LENGTH_UTILS)
        return false;
    ...
    while (name_start < path + len) {
        ...
        if (!name_end || name_end == name_start ||
            name_end > name_start + MAX_FOLDER_NAME_LENGTH_UTILS)
            return false;
...
const char *split_path(const char *path, char *component) {
    ...
    if (component) {
        ...
        assert(len >= 1 && len <= MAX_FOLDER_NAME_LENGTH_UTILS);
...
char *make_path_to_parent(const char *path, char *component) {
    ...
    if (component) {
        ...
        assert(component_len >= 1 && component_len <= MAX_FOLDER_NAME_LENGTH_UTILS);
